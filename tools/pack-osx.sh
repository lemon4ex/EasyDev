...
# substrate="$EASYDEV_PATH/MFrameworks/libsubstitute.dylib"
...
# 	"${EASYDEV_PATH}/bin/restore-symbol" -t "$APP_BINARY_PATH" -o "$APP_BINARY_PATH"_with_symbol
...
# 	/usr/bin/strip -t "$APP_BINARY_PATH" -o "$APP_BINARY_PATH".unsigned
...
if [[ ! -f "$APP_BINARY_PATH".insert ]]; then
	...
	"${EASYDEV_PATH}/bin/optool" install -c load -p "@executable_path/lib$TARGET_NAME.dylib" -t "$APP_BINARY_PATH"
